...
public class Command {
    ...
    private void setSize() {
        ...
        if (payload.getByteArray() != null)
            size = frame.getByteArray().length + frameAddress.getByteArray().length + protocol.getByteArray().length + payload.getByteArray().length;
        else
            size = frame.getByteArray().length + frameAddress.getByteArray().length + protocol.getByteArray().length;
        frame.setSize(size);
    }
    ...
    public void setFromCommandByteArray(byte[] byteArray) {
        ...
        if (protocol.getType() == 3)
            payload = new StateService();
        if (protocol.getType() == 13)
            payload = new StateHostInfo();
        if (protocol.getType() == 15)
            payload = new StateHostFirmware();
        if (protocol.getType() == 17)
            payload = new StateWifiInfo();
        if (protocol.getType() == 19)
            payload = new StateWifiFirmware();
        if (protocol.getType() == 22)
            payload = new StatePower_Device();
        if (protocol.getType() == 25)
            payload = new StateLabel();
        if (protocol.getType() == 33)
            payload = new StateVersion();
        if (protocol.getType() == 35)
            payload = new StateInfo();
        if (protocol.getType() == 45)
            payload = new Acknowledgement();
        if (protocol.getType() == 50)
            payload = new StateLocation();
        if (protocol.getType() == 53)
            payload = new StateGroup();
        if (protocol.getType() == 59)
            payload = new EchoResponse();
        if (protocol.getType() == 107)
            payload = new State_Light();
        if (protocol.getType() == 118)
            payload = new StatePower_Light();
        if (protocol.getType() == 121)
            payload = new StateInfrared();
