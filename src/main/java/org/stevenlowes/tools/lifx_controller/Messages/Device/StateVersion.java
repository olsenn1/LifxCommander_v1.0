...
public class StateVersion extends Payload {
    ...
    public void setFromCommandByteArray(byte[] byteArray) {
        String vendorBinStr = "";
        for (int i = 39; i > 35; i--) {
            vendorBinStr = vendorBinStr.concat(CommonMethods.convertByteToBinaryString(byteArray[i]));
        }
        vendor = Long.parseLong(vendorBinStr, 2);

        String productBinStr = "";
        for (int i = 43; i > 39; i--) {
            productBinStr = productBinStr.concat(CommonMethods.convertByteToBinaryString(byteArray[i]));
        }
        product = Long.parseLong(productBinStr, 2);

        String versionBinStr = "";
        for (int i = 47; i > 43; i--) {
            versionBinStr = versionBinStr.concat(CommonMethods.convertByteToBinaryString(byteArray[i]));
        }
