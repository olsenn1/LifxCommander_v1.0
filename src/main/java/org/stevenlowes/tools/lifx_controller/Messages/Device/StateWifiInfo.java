...
public class StateWifiInfo extends Payload {
    ...
    public void setFromCommandByteArray(byte[] byteArray) {
        String signalBinStr = "";
        for (int i = 39; i > 35; i--) {
            signalBinStr = signalBinStr.concat(CommonMethods.convertByteToBinaryString(byteArray[i]));
        }
        ...
        String txBinStr = "";
        for (int i = 43; i > 39; i--) {
            txBinStr = txBinStr.concat(CommonMethods.convertByteToBinaryString(byteArray[i]));
        }
        tx = Long.parseLong(txBinStr, 2);

        String rxBinStr = "";
        for (int i = 47; i > 43; i--) {
            rxBinStr = rxBinStr.concat(CommonMethods.convertByteToBinaryString(byteArray[i]));
        }
        rx = Long.parseLong(rxBinStr, 2);

        String reservedBinStr = "";
        for (int i = 49; i > 47; i--) {
            reservedBinStr = reservedBinStr.concat(CommonMethods.convertByteToBinaryString(byteArray[i]));
        }
